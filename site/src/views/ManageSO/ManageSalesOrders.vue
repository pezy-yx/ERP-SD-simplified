...
function resetSalesOrderDataTree() {
  salesOrderDataTree.forceSetValue({})
  const itemOverview = salesOrderDataTree.findNodeByPath(['itemOverview'])!
  itemOverview.children = itemOverview.children.filter(child=>child.name!='items')
  itemConditionKit.summonItemsNode(
    salesOrderDataTree,
    ['itemOverview', 'items']
  )
...
const performSalesOrderSearch = async () => {
  ...
  const queryData = salesOrderQueryTree.getValue();
...
async function handleExecute(currentStage: number, targetStage: number) {
  ...
  if (currentStage === 0) {
    if (onDisplayState.value) {
      ...
      await itemsTabQueryAll();
      const dataToSave = salesOrderDataTree.getValue();
...
const confirmQuotationInput = async () => {
  ...
  try {
    ...
    if (result.success && result.data.quotationData) {
      ...
      //   }
      // }
      salesOrderDataTree.findNodeByPath(['itemOverview'])?.forceSetValue(quotationData.itemOverview)
